...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const body = document.body;
  const dropdown = document.querySelector(".dropdown");
  const dropdownLink = document.querySelector(".dropdown-link"); // (only matters if you actually have this class)
  ...
  if (!handledCard) return;

  const navigateToHandled = (url) => {
    // Add pressed state (optional)
    ...
    // Cleanup in case navigation is interrupted
    setTimeout(() => {
  ...
  handledCard.addEventListener("click", (e) => {
    const url = handledCard.dataset.href; // ✅ now reads from data-href on the DIV
    if (!url) return;

    // Ripple effect (works because .project-card has position:relative + overflow:hidden)
  ...
  // Accessibility: allow Enter/Space to activate like a link
